...
-- Example seed device status
insert into public.device_status (id, organization_id, battery, connectivity, temperature, system_status)
select gen_random_uuid(), (select id from public.organizations limit 1), 85, 'Strong', 38, 'Active'
...
-- Organizations: users can select their own org; insert allowed for onboarding; update by admin only
...
-- Users: CRUD within same org; select limited to same org
...
-- Tables without org_id directly: tie via joins in application, keep selectable by all authenticated if needed
